    ...
    def __call__(self, request):
        ...
        if self.request.user:
            if self.request.user.is_authenticated:
                # User is logged in, so let's build a list of all his groups.
                for group in self.request.user.groups.all():
                    groups.append(group.pk)
            else:
                # User is not logged in -- put him in the everybody group
                group = Group.objects.get(name="Everybody")
                groups = [group.pk]

        self.searchqueryset = self.searchqueryset.filter(SQ(group_access__in=groups))
